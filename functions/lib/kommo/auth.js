...
function getRedirectUri() {
    var _a;
    return (_a = process.env.KOMMO_REDIRECT_URI) !== null && _a !== void 0 ? _a : "";
}
/**
...
function getAuthorizeBaseUrl() {
    var _a;
    const custom = (_a = process.env.KOMMO_OAUTH_AUTHORIZE_URL) === null || _a === void 0 ? void 0 : _a.trim();
    if (custom)
...
function handleAuthRedirect(req, res) {
    ...
    if (!clientId || !redirectUri) {
        ...
        res.status(500).json({
            error: "KOMMO_CLIENT_ID or KOMMO_REDIRECT_URI not configured. " +
    ...
    console.log("[Kommo OAuth debug] redirect_uri (callback URL sent to Kommo, must match integration Redirect URI):", redirectUri);
    console.log("[Kommo OAuth debug] client_id (prefix):", clientId.length > 8 ? `${clientId.slice(0, 8)}…` : clientId);
    ...
    console.log("[Kommo OAuth debug] redirect_uri as query param on that URL:", authUrl.searchParams.get("redirect_uri"));
...
async function handleAuthCallback(req, res) {
    ...
    console.log("[Kommo OAuth debug] Callback: exchanging code using redirect_uri:", redirectUri);
    try {
        const tokens = await (0, tokenManager_1.exchangeCodeForTokens)(code, clientId, clientSecret, redirectUri);
